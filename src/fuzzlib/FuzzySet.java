 ...
 * 
 * @author PrzemysĂ„Ä…Ă˘â‚¬Ĺˇaw KudĂ„Ä…Ă˘â‚¬Ĺˇacik
 */
public class FuzzySet {
	...
	public FuzzySet(String _id, String _des) {
		...
		// TODO: Co z tym min i max ? nie powinny byÄ‚â€žĂ˘â‚¬Ë‡ standardowo na 0 ??
		min_val = 1000000;
		max_val = -1000000;
		tnorm = new TNMin();
		snorm = new SNMax();
	...
	// TODO: wyrzuciÄ‚â€žĂ˘â‚¬Ë‡ ponizsze metody z klasy FuzzySet (nie dotyczÄ‚â€žĂ˘â‚¬Â¦ce stricte zbioru
	// rozmytego)
	...
	private static void _calcIsTruthFunction(FuzzySet outFun, FuzzySet PSet, FuzzySet QSet, double minDY,
		...
		if ((Ps.x > Pp.x) && (Ps.x < Pk.x)) {
			...
			// prawa strona podzialu do przetworzenia pĂ„ĹąÄąÄ˝Ă‹ĹĄniej
			// ZMIANA 22.06
		...
		while (!STOP) {
			...
			// - zdarza sie ze srodek jest zgodny aproksymacjĂ„ĹąÄąÄ˝Ă‹ĹĄ na dwoch
			// punktach,
			// ale funkcja jest np. schodkowa (i schodek przypada wlasnie w
			// srodku)

			if (dy < minDY && dx >= minDX) { // jesli dy dobre i dx w porzĂ„ĹąÄąÄ˝Ă‹ĹĄdku
			...
			if (dy < minDY || dx < minDX) {
				...
				// (najlepiej zrealizowac dodawanie na liĂ„ĹąÄąÄ˝Ă‹ĹĄcie sortowanej - NA
				// RAZIE AddPoint bo juĂ„ĹąÄąÄ˝Ă‹ĹĄ jest)
				outFun.addPoint(Ps.x, Ps.y);
				// wrzuc Ps, Pk na STOS; - prawa strona podzialu do
				// przetworzenia pĂ„ĹąÄąÄ˝Ă‹ĹĄniej
				// ZMIANA 22.06
				st.push(new SPoint(Ps));
				st.push(new SPoint(Pk));
		...
		// NA RAZIE WYKORZYSTUJĂ„ĹąÄąÄ˝Ă‹ĹĄ FUNKCJĂ„ĹąÄąÄ˝Ă‹ĹĄ AddPoint klasy FuzzySet - nieefektywne
	...
	public static void calcCompoundTruthFunction(FuzzySet outFun, FuzzySet trFunA, short composition_type,
		...
		// Algorytm np dla zlaczenia iloczynem daje "poszarpanĂ„ĹąÄąÄ˝Ă‹ĹĄ" funkcjĂ„ĹąÄąÄ˝Ă‹ĹĄ
		// wyjsciowĂ„ĹąÄąÄ˝Ă‹ĹĄ gdy
		// zbior A osiaga maksimum dla wartosci bliskim zeru (falsz-falsz
		// absolutny)
		// oraz gdy zbior B "podchodzi" w stronĂ„ĹąÄąÄ˝Ă‹ĹĄ prawdy.
		...
		default:
			...
			// kiedyĂ„Ä…Ă˘â‚¬Ĺź przy wykonywaniu zĂ„Ä…Ă˘â‚¬Ĺˇozenia dla PRoduct TNorm
			// funkcja byĂ„Ä…Ă˘â‚¬Ĺˇa poszarpadna.
			// Problem przestal sie pojawiac gdy zamieniono funkcje
			// if ( itrA.getMaxPoint().x > 0.3)
			// trA = &itrB; trB = &itrA;
			// ALE to byĂ„Ä…Ă˘â‚¬Ĺˇo w dedykowanej metodzie findMaxAandBforMembership obiektu dla
			// TNormy Product
			// - teraz jest jedna wspĂ„â€šÄąâ€šlna metoda. Jakby problem wystÄ‚â€žĂ˘â‚¬Â¦piĂ„Ä…Ă˘â‚¬Ĺˇ to nalezy znow
			// funkcje zamienic..
	...
	private static double _findMaxQforImpl(double n, FuzzySet QSet, Norm impl, TNorm tnorm) {
		...
		// Operacja sup(TNorm) - projekcja na 'n' przeciĂ„ĹąÄąÄ˝Ă‹ĹĄcia funkcji prawdy z
		// implikacjĂ„ĹąÄąÄ˝Ă‹ĹĄ
